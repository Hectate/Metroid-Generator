...
class StencylPreloader extends NMEPreloader
{
		...
		if(true && true)
		{
			...
			if(true)
			{
				try
				{
					...
					BitmapData.loadFromBase64(data, "png", function(bitmapData:BitmapData) 
					{
	...
	#if(flash || js) override #end public function getBackgroundColor():Int
	{
		return 0x336699;
	}
	
	#if(flash || js) override #end public function getWidth():Float
	{
		return 640.0;
	}
	
	#if(flash || js) override #end public function getHeight():Float
	{
		return 480.0;
	}
	
	#if(flash || js) override #end public function onInit()
	{
	}
	
	#if(flash || js) override #end public function onLoaded()
	{
		if(adFinished)
		{
			#if(flash || js)
	...
	#if(flash || js) override #end public function onUpdate(bytesLoaded:Int, bytesTotal:Int)
	{
